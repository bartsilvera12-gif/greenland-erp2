...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertBase = {
      ...
      email_secundario:     typeof email_secundario === "string" && email_secundario.trim() ? email_secundario.trim() : null,
      direccion:            direccion?.trim() || null,
      ciudad:               ciudad?.trim() || null,
      pais:                 pais?.trim() || null,
      condicion_pago:       condicion_pago?.trim() || null,
      credito_medio:            typeof credito_medio === "string" && credito_medio.trim() ? credito_medio.trim() : null,
      credito_cuotas_cantidad:  Number.isFinite(Number(credito_cuotas_cantidad)) && Number(credito_cuotas_cantidad) > 0 ? Math.trunc(Number(credito_cuotas_cantidad)) : null,
      credito_monto_cuota:      Number.isFinite(Number(credito_monto_cuota)) && Number(credito_monto_cuota) > 0 ? Number(credito_monto_cuota) : null,
      credito_primera_cuota:    typeof credito_primera_cuota === "string" && /^\d{4}-\d{2}-\d{2}$/.test(credito_primera_cuota) ? credito_primera_cuota : null,
      credito_intervalo_dias:   Number.isFinite(Number(credito_intervalo_dias)) && Number(credito_intervalo_dias) > 0 ? Math.trunc(Number(credito_intervalo_dias)) : null,
